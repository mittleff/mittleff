(define-module (mittleff algorithm)
  ...
  #:use-module (mittleff rgamma)
  #:use-module (mittleff erfc)
...
(define* (series a b z #:key (prec *default-precision*) (asymptotic? #f))
  (let* ((acc (accuracy-from-prec prec))
         (kmin (if asymptotic? 1 0))
         (kmax (if asymptotic?
...
(define* (taylor a b z #:key (prec *default-precision*))
  (series a b z #:prec prec #:asymptotic? #f))

(define* (asymptotics a b z #:key (prec *default-precision*) (radius *taylor-radius*))
  (asymptotics-aux a b z #:region
                   (cond
                    ((in-region-G1? z a #:prec prec #:taylor-radius radius) 1)
                    ((in-region-G2? z a #:prec prec #:taylor-radius radius) 2)
                    ((in-region-G3? z a #:prec prec #:taylor-radius radius) 3)
                    ((in-region-G4? z a #:prec prec #:taylor-radius radius) 4)) #:prec prec))

(define* (asymptotics-aux a b z #:key (region 1) (prec *default-precision*))
  (define (c th)
  ...
  (let ((fac1 (/ (rho region) a))
        (fac2 (expt z (/ (- 1 b) a)))
        (fac3 (exp (expt z (/ a)))))
    (- (* fac1 fac2 fac3) (series a b z #:prec prec #:asymptotic? #t))))
...
(define* (integral-rep a b z #:key (region 5) (prec *default-precision*) (radius *taylor-radius*))
  (integral-rep-aux a b z #:region
                    (cond
                     ((in-region-G5? z a #:prec prec #:taylor-radius radius) 5)
                     ((in-region-G6? z a #:prec prec #:taylor-radius radius) 6)) #:prec prec))

(define* (integral-rep-aux a b z  #:key (region 5) (prec *default-precision*))
  (let ((acc (accuracy-from-prec prec)))
    ...
    (let ((Ai (cond ((= region 5) (fn-a z a b 0)) ((= region 6) 0)))
          ...
          (Ci (if (> b 1) (quad (lambda (ph) (fn-c ph a b z 1/2)) (- (Phi region)) (Phi region) #:prec prec) 0)))
      (+ Ai Bi Ci))))
